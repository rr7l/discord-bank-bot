...
for (const fontPath of fontCandidates) {
  console.log(`🔍 تجربة مسار الخط: ${fontPath}`);
  ...
  try {
    ...
    if (loaded) {
      ...
      console.log(`✅ تم تحميل الخط العربي من: ${fontPath}`);
      break;
    }
  } catch (error) {
    console.error(`❌ فشل تحميل الخط: ${error.message}`);
  }
}

if (!fontLoaded) {
  console.error('❌ لم يتم العثور على خط NotoSansArabic');
  console.error('📁 تأكد أن الملف موجود داخل مجلد fonts');
}
